...
class PDQuadraticNet(nn.Module):
    ...
    def __init__(
        ...
        self.f = MLP(
            in_dim, in_dim, hidden_layers, hidden_activation, None, init, gain
        )
        self.Q = nn.Parameter(torch.eye(in_dim) + 0.1 * torch.ones((in_dim, in_dim)))
        # self.theta = nn.Parameter(torch.ones(in_dim, in_dim))
        self.g = MLP(
            in_dim, 1, hidden_layers, hidden_activation, nn.ReLU(), init, gain
        )
    ...
    def quadratic_matrix(self) -> torch.Tensor:
        # return torch.matmul(self.theta, self.theta.t())
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.quadratic(x) + (self.f(x)**2).sum(dim=1, keepdim=True) + self.g(x)
